...
%>				
					<Tr>
						<Th align="left">Type of Vendor</Th>
						<Td colspan="3">
						<div>
<%						
...
%>						
									<input type="checkbox" name="vendorType" class="form-check-input"  value="<%=me.getKey()%>" <%=chksel%>>&nbsp;&nbsp;&nbsp;<%=me.getValue()%>&nbsp;&nbsp;&nbsp;		
<%
							}
%>								
						</div>
...
<%
				//out.println("Payment_Terms>>>>>>>>>>>>>"+Payment_Terms);
				//Payment_Terms="BT01";
...
%>
		   ...
		   <!--My attchment task-2-->
<%
		if("4101".equals(compCode))
		{
%>
		   <table class="table table-bordered">
		<Tr>
			<Th align="left" width="30%">W8Form <%if("IN".equals(vendType) && !"IN".equals(country)){%><font color="red">*</font><%}%></Th>
			<Td width="20%">
...
%>
		 ...
		 <table class="table table-bordered">
		<Tr>
			<Th align="left" width="30%">International Form<%if("IN".equals(vendType) && !"IN".equals(country)){%><font color="red">*</font><%}%></Th>
			<Td width="20%">
				<input type="text" class="form-control validate[required]"  name="INTAttach" id="INTAttach" value="<%=checkNull(INTAttach)%>" maxlength="20" >
			</Td>
			<td width="20%"><button type="button"  value = "VENDINT"  class="btn btn-primary uploadDialogBtn" ><i class="fa fa-upload"></i>&nbsp;Upload</button></td>
			<td width="30%">
			 <table class="VENDINTTABLE table  table-bordered dt-responsive nowrap" id="VENDINTTABLE" name="VENDINTTABLE" >
<%
			Vector itemIndINT=retUploadDocs.where(new String[]{"EUD_DOC_TYPE"},new String[]{"VENDINT"});
			for(int v1=0;v1<itemIndINT.size();v1++)
			{	
				int index1 = -1;
				try{
				index1 = Integer.parseInt(itemIndINT.get(v1)+"");	}
				catch(Exception e){}				
				if(index1>=0){
...
%>
	...
	function isValidAttachments()
	{
		...
		//GETTING International form Attachment		
		var INTTabLen = $('#VENDINTTABLE tr').length;		
		var INTAttach = document.myForm.INTAttach.value;
		...
		if(vendType=="IN")
		{
			...
			if("IN"==vendType && INTAttach=="")
			{
				//alert("INT FORM");
				//if(compCode=="7300" || compCode=="4101" || compCode=="9000")
				{
					$('#INTAttach').validationEngine('showPrompt', 'Please provide INT Form Details', '');
					return false;
				}
			}
			...
			if("IN"==vendType && INTTabLen==0 )
			{
				//if(compCode=="7300" || compCode=="4101" || compCode=="9000")
				{
					$('#VENDINTTABLE').validationEngine('showPrompt', 'Please provide INT Form file', '');
					return false;
				}
			}
			...
			if("IN"==vendType && INTAttach=="" && INTTabLen==0)
			{
				//if(compCode=="7300" || compCode=="4101" || compCode=="9000")
				{
					$('#INTAttach').validationEngine('showPrompt', 'Please provide INT Form Details', '');
