...
<%
	...
	out.println("vendSubmit_JS_L>>>>>>>"+vendSubmit_JS_L);	
...
%>				
				<Tr>
					...
					<Td>
						<Select name="INCO_Terms" id="INCO_Terms" class="form-control validate[required]" data-errormessage-value-missing="Please Select Inco Terms">		
...
%>
							<input type="radio" name="pmntMode" id="pmntMode"  class="form-check-input validate[required]" data-errormessage-value-missing="Please Select type of Payment" value="<%=me_pmnt.getKey()%>" <%=chksel%> onChange="showPmntMode(this)">&nbsp;&nbsp;&nbsp;<%=me_pmnt.getValue()%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<%
					}
%>
			</Tr>
...
%>
					...
					<Th align="left"> Recon Account <font color="red">*</font> <font color="red">*</font></Th>
					<Td>
...
<%
	...
	if(("PP".equals(userRole) || "BY".equals(userRole) && showSubmit && compCode=="4101" )|| "4".equals(userType))
	{
%>
		$('#paymntTerms').attr('disabled',false);
<%
	}
...
%>
		   $(function tabSave() {
			...
			var docId=$("#docId").val();
			var isPPEditable = '<%=isPPEditable%>';
			var isEditable='<%=isEditable%>';
			if(tabVal==2 && isPPEditable=='true')
			{
				
				var paymntTerms=$("#paymntTerms").val(); 
				

				$.ajax({
				type: 'post',
				url: 'ezSaveVendRegOthMat.jsp',
				data: $('#tabs-'+tabVal).find("select, textarea, input").serialize()+"&docId="+docId+"&paymntTerms="+paymntTerms,
