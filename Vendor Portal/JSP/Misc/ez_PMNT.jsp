...
<input type="hidden" name="pmntObj" id="vendName" value="<%=pmntObj%>">
<input type="hidden" name="Payment_Terms" id="docId" value="<%=Payment_Terms%>">
				...
				<div class="box box-primary">
					...
					<div class="box-body">
						...
						<tr>
							    ...
							    <td>
								<input type="number" class="form-control validate[required]" data-errormessage-value-missing="Please Enter Postal Code" name="postalCode" value="<%=checkNull(PostalCode)%>" maxlength="50">
							    </Td>
						...
						<tr>
							    ...
							    <td>
								<Select name="country" id="country" class="form-control validate[required]" data-errormessage-value-missing="Please select Country Name" name="country">
...
%>
								...
								<td>
								<input type="text" class="form-control validate[required]" data-errormessage-value-missing="Please Enter Mobile Number" name="mobNum" value="<%=checkNull(MobileNumber)%>" maxlength="50">
								</Td>
						</tr>
						<tr>
						...
						<tr>
							   <th align="left"><%=Addr_Line5_L%></th>
							    ...
							    <td>
								<Select name="state" id="state" class="form-control validate[required]" data-errormessage-value-missing="Please Select State" name="state">
						...
						<tr>
							   <th align="left"><%=email_addr1_L%><font color="red">*</font></th>
							    <td>
								<input type="text" class="form-control validate[required,custom[email]]" data-errormessage-value-missing="Please Enter Address" name="email_1" value="<%=checkNull(EamilAddress1)%>" maxlength="50">
							    </Td>
...
%>
				...
				<Tr>
					...
					<Td>
						<Select name="INCO_Terms" id="INCO_Terms" class="form-control select2 validate[required]" data-errormessage-value-missing="Please Select Inco Terms">	
...
%>					
				...
				<Tr>
					<Th align="left"><%=copy_of_pan_L%></Th>
					<Td>
					...
					<Td>
						<input type="text" class="form-control validate[required]" data-errormessage-value-missing="Please enter Pan Name"  id="PAN_Name" name="PAN_Name"  value="<%=checkNull(PAN_Name)%>" maxlength="100">
					</Td>								
				</Tr>
...
<%
				...
				{	
					...
					String payTm1=pmntObj.getFieldValueString(j,"EMKV_VALUE1").trim();
					if(payTm1==null || "null".equals(payTm1))payTm1="";
					if("0".equals(MSME_Type) && !"M".equals(payTm1))
						continue;
...
%>						
				...
				<Tr id="vendUnit" style="display:<%=dispUnit%>;">
					<Th align="left"><%=buss_Unit_Name_L%><font color="red">*</font></Th>
					<Td colspan="3">
...
<%
							...
							if("Y".equals(vend_Trans)) 
							{
								...
								chkTansNo="checked";
%>						
							<input class="form-check-input" type="radio"   name="vend_Trans"  value="Y" onChange="showDtls(this)" <%=chkTansYes%>>&nbsp;&nbsp;&nbsp;Yes&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<input class="form-check-input" type="radio"   name="vend_Trans"  value="N" onChange="showDtls(this)" <%=chkTansNo%>>&nbsp;&nbsp;&nbsp;No&nbsp;
						</div>
					</Td>
				</Tr>
				<Tr id="vendDtls" style="display:<%=dispDtls%>;">
					<Th align="left"><%=vend_Details_L%><font color="red">*</font></Th>
					<Td colspan="3">
...
%>					
				...
				<Tr id="siteViDt" style="display:<%=dispSite%>;">
					<Th align="left"><%=site_Visit_On_L%></Th>
					<Td colspan="3">
	...
	<table class="table table-bordered">
			<Tr>
				...
				<td width="20%"><button type="button"  value = "VENDITR"  class="btn btn-primary uploadDialogBtn" ><i class="fa fa-upload"></i>&nbsp;Upload</button></td>
				<td width="30%">
...
%>
		...
		<Tr>
				...
				<td width="20%"><button type="button"  value = "VENDMOA"  class="btn btn-primary uploadDialogBtn" ><i class="fa fa-upload"></i>&nbsp;Upload</button></td>
				<td width="30%">
...
%>
		  	<table class="table table-bordered">
				<Tr>
					<Th align="left" width="30%"><%=vend_Creation_Form_L%></Th>
					<Td width="20%">
...
%>
				...
				<Tr>
					...
					<td><button type="button"  id="VENDREGCHEQ" value = "VENDREGCHEQ"  class="btn btn-primary uploadDialogBtn" ><i class="fa fa-upload"></i>&nbsp;Upload</button></td>
					<td>
...
%>
				<Tr>
					<Th colspan="2" align="left"><%=vendAccGrp_L%></Th>
					<Td colspan="2">
...
%>
		<Table align="center" width="100%" border=1 borderColorDark=#ffffff borderColorLight=#006666 cellPadding=2 cellSpacing=0 >
			<Tr <%=trBGColor%>>
				<Th align=left ><B>Please Enter Comments</B></Th>
			</Tr>
			<Tr>	
...
%>
	...
	<script>
		...
		function funSubmit(status)
		{
			...
			$.ajax({
				...
				if(result.split("#")[1] == "Q")
				{
				    alert('<%=vendReg_JS_L%>'+qcfNo+'<%=cantAppr_JS_L%>');
				    return;
				}
				else
				{
	...
	function showMSME(fieldObj)
	{
		...
		{
			document.getElementById("paymntTerms").style.display="";
		}
		else
		{
			document.getElementById("paymntTerms").style.display="none";
		}
		//alert("hii2::::"+eval('document.myForm.msme_Type').value);
		var xmlhttp;
		...
		{
			//alert("ready state::::latest:::"+xmlhttp.readyState);
			//alert("status:::: LATEST:::::"+xmlhttp.status);
			if (xmlhttp.readyState==4 && xmlhttp.status==200)
			{
				//alert("status"+xmlhttp.responseText);
				if(xmlhttp.responseText!=null)
				{
					if(eval('document.myForm.msme_Type').value!="")
					{
						 document.getElementById("paymntTerms").innerHTML =xmlhttp.responseText;
						 $('.select2').select2()	
					}
		...
		xmlhttp.open("post","ezGetPaymentTerms.jsp?fieldVal="+fieldVal,false);
		xmlhttp.send();	
}
