...
<%
	...
	String selDocType = checkNull(request.getParameter("selDocType"));
	String poNumber = checkNull(request.getParameter("poNumber"));
	String chkURL=selDocType+"_"+vendorDocId+".pdf";
	//out.println("selDocTypeselDocType"+selDocType);
	String flag = checkNull(request.getParameter("flag"));
	String matVendorDocId = vendorDocId+"M";
	
	String uploadFilesQry="";
	/*
	if("FREIGHT".equals(selDocType) || "LOADING".equals(selDocType) || "RHLOADING".equals(selDocType) || "UNLOADING".equals(selDocType) || "RHUNLOADING".equals(selDocType))
	uploadFilesQry="SELECT * FROM EZC_UPLOAD_FILES WHERE EUF_DOC_ID IN('"+vendorDocId+"') ORDER BY EUF_ATTACHED_DATE";
	else
	*/
	uploadFilesQry="SELECT * FROM EZC_UPLOAD_FILES WHERE EUF_DOC_ID IN('"+vendorDocId+"') ORDER BY EUF_ATTACHED_DATE";
	...
	miscParams.setQuery(uploadFilesQry);
...
function openViewUploadWindow(serverFile,clientFile)  
{
	...
	window.open("/HSIL/Uploads/"+sFile,"newWin","titlebar=yes")
}
function funOpenFile(serverFile)
{
  ...
  sFile = sFile.substring(0,sFile.length-1)
  //window.open("../Misc/ezViewFile.jsp?filename=<%=serverFilePathDir%>MATERIAL/"+sFile,"newWin","titlebar=yes")
  document.location.href="../Misc/ezViewFile.jsp?filename=<%=serverFilePathDir%>MATERIAL/"+sFile
}
</script>
</head>
<body>
...
%>
	<div class="col-md-10 col-md-offset-1">
		 <table id="dataTab" class="table table-hover table-condensed table-bordered" >
			...
			<%
					if(chkURL.equalsIgnoreCase(fileName) || fileName.equals("MaterialService_"+poNumber+".pdf"))
					{
			%>
					<td><%=fileName%></td>
			<%
			...
			%>
					<td >
					<!--<a href="javascript:openViewUploadWindow('<%=attachedServerFile%>','<%=attachedClientFile%>')" style="background-color:white"><%=fileName%></a>-->
					<a href="javascript:funOpenFile('<%=attachedServerFile%>')" style="background-color:white"><%=fileName%></a>
