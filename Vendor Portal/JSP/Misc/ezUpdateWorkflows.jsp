...
<%
	...
	String selQuery = "SELECT *, EWPC_ROLE AS ROLE, EWPC_GROUP AS GROUP_NAME, (SELECT GROUP_CONCAT(B.EU_ID) FROM EZC_WF_WORKGROUP_USERS A, EZC_USERS B WHERE A.EWWU_USER = B.EU_ID AND A.EWWU_GROUP = EWPC_GROUP AND A.EWWU_USER IN (SELECT EUDKV_USER_ID FROM EZC_USER_DEF_KEY_VALUES WHERE EUDKV_KEY = 'CATEGORY' AND EUDKV_VALUE = '"+plant+"#"+category+"') GROUP BY A.EWWU_GROUP) AS USER_IDS, (SELECT GROUP_CONCAT(B.EU_FIRST_NAME) FROM EZC_WF_WORKGROUP_USERS A, EZC_USERS B WHERE A.EWWU_USER = B.EU_ID AND A.EWWU_GROUP = EWPC_GROUP AND A.EWWU_USER IN (SELECT EUDKV_USER_ID FROM EZC_USER_DEF_KEY_VALUES WHERE EUDKV_KEY = 'CATEGORY' AND EUDKV_VALUE = '"+plant+"#"+category+"') GROUP BY A.EWWU_GROUP) AS USER_NAMES FROM EZC_WF_PRICING_CONDITIONS WHERE EWPC_TEMPLATE IN (SELECT EWKM_TEMPLATE FROM EZC_WF_KEY_MAP WHERE EWKM_COMP_CODE = '"+compCode+"' AND EWKM_PLANT = '"+plant+"' AND EWKM_CATEGORY = '"+category+"' AND EWKM_DOC_TYPE = '"+docType+"') ORDER BY EWPC_LEVEL";
...
<head>
	...
	/*#workflowTable tbody tr:nth-child(odd){
		background-color: #f9f9f9;
	}
	
	#workflowTable tbody tr:nth-child(even){
		background-color: #ffffff;
	}*/
	
	#workflowTable tbody tr:hover{
	...
	function addRow(rowKey) {
		...
		var unsavedRows = document.querySelectorAll("tr.new-rowww-highlight");
		unsavedRows.forEach(function(row){
			row.remove();
		});
		
		var condition="";
		var price="";
		var role = currentRow.querySelector("td:nth-child(2)").innerText.trim();
		<% if (!"VNR".equals(docType)){ %>
			condition = currentRow.querySelector("td:nth-child(5)").innerText.trim();
			price = currentRow.querySelector("td:nth-child(6)").innerText.trim();
		<%}%>
		...
		newRow.innerHTML =
					...
					'</td>' +
					'<td id="tdParticipantName' + newRowId + '"></td>';
		<% if(!"VNR".equals(docType)){%>
		newRow.innerHTML += 
		...
		if(xmlhttp.readyState === 4){
			...
			allRows.forEach(function(row){
				var idInput = row.querySelector("input[id^='userId'], input[id^='WFParticipantHidden']");
				if(idInput){
			...
			//Do not preselect anything; leave name field blank
			...
			$(select).on('select2:select', function(e){
				...
				document.getElementById("tdParticipantName" + newRowId).innerText = selectedValue ? selectedName : "";
				document.getElementById("WFParticipantHidden" + newRowId).value = selectedValue;
			});

			// Handle deselection (if user re-selects default)
			$(select).on('change', function () {
				var selectedValue = this.value;
				if(selectedValue === ""){
					document.getElementById("tdParticipantName" + newRowId).innerText = "";
					document.getElementById("WFParticipantHidden" + newRowId).value = "";
				}
			});
	...
	function deleteRow(index){
		...
		var userId = document.getElementById("WFParticipantHidden" + index)?.value || document.getElementById("userId" + index)?.value || "";

		var plant = "<%= plant %>";
		var category = "<%= category %>";

		var isNewRow = row.classList.contains("new-rowww-highlight");
		...
		allRows.forEach(function(row){
			if(!row.classList.contains("new-rowww-highlight")){
				saveRowsCount++;
			}
		});
		
		if(!row.classList.contains("new-rowww-highlight") && saveRowsCount <= 1){
			alert("At least one user is required at each level. Add another user before removing this user.");
			return;
		}
		...
		var url = "ezDeleteWorkflowUser.jsp?plant=" + encodeURIComponent(plant) + "&category=" + encodeURIComponent(category) + "&userId=" + encodeURIComponent(userId);
	...
	function saveRow(index){
		...
		if(!userId || userId === ""){
			alert("Please select a user to add to WF.");
			var dropdown = document.getElementById("WFParticipantSelect" + index);
			if(dropdown){
		...
		var url = "ezUpdateWorkflowCondition.jsp?plant=" + encodeURIComponent(plant) + "&category=" + encodeURIComponent(category) + "&userId=" + encodeURIComponent(userId);
