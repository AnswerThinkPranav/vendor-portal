...
<%//@ include file="ezGetNACompCode.jsp"%>
...
%>					
				...
				<%
							if("3001".equals(plant)) 
							{
				%>
								<Tr>
									<Th align="left">VAT </Th>
									<Td>
										<input type="text"  id="Tax_vat" name="Tax_vat"  value="<%=checkNull(Tax_vat)%>" maxlength="100"  data-errormessage-value-missing="Please Enter VAT Value">
									</Td>				
...
%>						
				...
				<Tr id="vendUnit" style="display:<%=dispUnit%>;">
					<Th align="left"><%=buss_unit_L%><font color="red">*</font></Th>
					<Td colspan="3">
...
%>					
				...
				<Tr id="sitePerson" style="display:<%=dispSite%>;">
					<Th align="left"><%=name_of_Person_L%></Th>
					<Td colspan="3">
				...
				<Tr id="siteViDt" style="display:<%=dispSite%>;">
					<Th align="left"><%=site_visited_L%></Th>
					<Td colspan="3">
...
%>
		  ...
		  <input type="hidden" name="tabVal5" id="tabVal5" value="5" >
		  	...
		  	<table class="table table-bordered">
			<Tr>
					<Th align="left"><%=Acc_Person_L%> </Th>
					<Td>
						<input type="text" class="form-control"   name="accnt_Per" value="<%=checkNull(Accnt_Person)%>" maxlength="100">
					</Td>
					<Th align="left"><%=vendEmail_L%></Th>
					<Td>
					<input type="text" class="form-control "   name="accnt_Email" value="<%=checkNull(Accnt_Email)%>" maxlength="100">
					</Td>
					
				</Tr>		  	
				<Tr>
					<Th align="left"><%=mobile_L%> </Th>
					<Td>
					<input type="text" class="form-control "   name="accnt_Mobile" value="<%=checkNull(Accnt_Mobile)%>" maxlength="100">
					</Td>				
					<Th align="left"><%=IFSC_Code_L%> </Th>
					<Td>
<%
					if("Y".equals(showStr) && "VAR".equals(userRole))
					{
%>					
						<input type="text" class="form-control "   name="rtgs_Code" id="rtgs_Code" style="background-color:#ff0000bf" value="<%=checkNull(RTGSCode)%>" maxlength="100" onChange="chkIFSC(this)">
<%
					}else{
%>
...
%>
				...
				<Tr>
					<Th align="left"><%=bank_Name_L%> </Th>
					<Td>
						<input type="text" class="form-control "   name="bankName" value="<%=checkNull(BankName)%>" maxlength="100">
					</Td>
					<Th align="left"><%=branch_L%></Th>
					<Td>
						<input type="text" class="form-control "   name="bank_Branch" value="<%=checkNull(Branch)%>" maxlength="100">
					</Td>
								
				</Tr>				
				<Tr>
					<Th align="left"><%=account_Num_L%> </Th>
					<Td>
						<input type="text" class="form-control "   name="accnt_Number" value="<%=checkNull(Account_Number)%>" maxlength="100">
					</Td>
...
%>
				...
				<Tr>
					<Th align="left"><%=recon_Acc_Type_L%> </Th>
					<Td>
						
						<Select name="recType" id="recType" class="form-control select2 " data-errormessage-value-missing="Please Select Recon Account Type">	
...
%>
			<tr>
				<td align="left" width="95%"><input type="hidden" name="fileItemH" value="<%=retUploadList.getFieldValueString(r,"EUD_FILE_NAME")%>"><a href="javascript:downloadFile1('<%=retUploadList.getFieldValueString(r,"EUD_ITEM_NO")%>','<%=retUploadList.getFieldValueString(r,"EUD_FILE_NAME")%>')"><%=retUploadList.getFieldValueString(r,"EUD_FILE_NAME")%></a></td>
...
%>
	...
	<script>
							...
							else
							{
								...
								else
								{
									...
									if(role == "VAR")
									{
										//alert("role>>>"+role);
										$.ajax({
											url: 'ezPrePostImpVendorToSAP.jsp', 
											data: { "docId":qcfNo},
											type: 'POST',
											success: function(result) {
											result=$.trim(result);  
		...
		function downloadFile1(upItemNo,fileName)
		{
