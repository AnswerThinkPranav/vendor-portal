...
String  ORT01Bank="",BKREF="";
	...
	String selVendorTemp="0000000000"+vendor;
	selVendorTemp=selVendorTemp.substring((vendor).length(),selVendorTemp.length());
	generalParams.setVendor(vendor+"','"+selVendorTemp);
	
	if("REJECTED".equals(status))
	generalParams.setStatus("REJECTED");
	else if("POSTED".equals(status) || "CLOSED".equals(status))
...
JCoParameterList impParam = function.getImportParameterList();
			...
			//int expCoun         = expTable.getNumRows();
					...
					//String MANDT0       =  (String)expTable.getValue("MANDT");
					 ...
					 ezc.ezcommon.EzLog4j.log(":::::LIFNR:::::::"+LIFNR,"I");
			...
			//out.println(retTable3);
			//out.println(retTable4);
			...
			if(retCoun > 0)
						...
						if(retCoun1 > 0)
						{
							do
							{
								...
								try
								{
									CLASSIF       =  checkNull((String)retTable1.getValue("VEN_CLASS"));
									CLASSIF_DESC  =  checkNull((String)classHT.get(CLASSIF));
								}catch(Exception e){}
...
//out.println(certificateDate+"::::::::certificateDate");
	...
	String email2 = "";
		...
		String email2Style="";
		try{
		email2 =checkNull(emailRetObj.getFieldValueString(0,"EVE_EMAIL"));
		}catch(Exception e){}
	out.println(email2+"::::email2::::::"+email);
...
String contPers2 = "";
	...
	ReturnObjFromRetrieve retMinInd1	=  null;
	int    MinIndCnt1	=  0;
	String minIndi = "";
	...
	String classiStyle="";
	try{
	classi ="02";//checkNull(exciseDataRetObj.getFieldValueString(0,"EVED_CLASSIFICATION"));
	}catch(Exception e){}
	...
	<Div class=" col-md-12 col-sm-12 col-xs-12"> 
		<Div class="box box-info collapsed-box">
			...
			<Div class="box-body" style="display: block;">
				...
				<Tr>
					...
					<Td colspan="4">
						<Input readonly  type="text" style="text-transform: uppercase;" name="vendCode" id="vendCode" value="<%=Integer.parseInt(LIFNR)%>"  maxlength="10" required>
					</Td>
					<Th align=left>Vendor Name</Th>
					<Td colspan="4">
					<Input readonly  type="text" style="text-transform: uppercase;width: 350px;" name="venName" id="venName" value="<%=NAME1+NAME2%>"  maxlength="35">
		...
		%>
			<Tr>
				...
				if(MinIndCnt1>0)
				{
				mindkIndex = retMinInd1.getRowId("EMKV_KEY",MINDK);
				
				if(mindkIndex>=0)
				MINDK=checkNull(retMinInd1.getFieldValueString(mindkIndex,"EMKV_VALUE"));
				}
				%>
					<%=MINDK%>				
				</td>
				...
				<%
					...
					if(MinIndCnt1>0)
					{
					mindkIndex = retMinInd1.getRowId("EMKV_KEY",minIndi);

					if(mindkIndex>=0)
					minIndiDesc=checkNull(retMinInd1.getFieldValueString(mindkIndex,"EMKV_VALUE"));
					}
				%>
		...
		%>
			<Tr>
				...
				<%
					if(classiChanged)
					{
					DB_CLASSIF_DESC  =  checkNull((String)classHT.get(classi));
				%>
...
<%
//out.println(bankDtlsRetObj.toEzcString());
...
String bankACCodeStyle="";
			...
			{			
				...
				bankIndex=bankListRetObj.getRowId("KEY",keyStr);
...
%>
			...
			<input type="hidden" name="bankCountry<%=keyStr%><%=BANKN%>" value="IN">
		<%
			if(bankNameChanged)
			{
		%>
		<Tr>	
			...
			<%
				...
				{
			%>
				<input type="hidden" name="bankName<%=keyStr%><%=BANKN%>" value="<%=BANKA%>"><%=BANKA%>
			<%
				}
			%>
			</Td>
						...
						for (int i=0;i<StateCnt;i++)
						 {
						 	...
						 	if(statekey.equals(bankRegion))
						 	{
						 	statevalue = retState.getFieldValueString(i,"EMKV_VALUE1");
							break;
							}
						 }
						 if(!"".equals(statevalue))statevalue=statevalue+"--";
%>
				<%=statevalue%><%=bankRegion%>				
			</td>
			<Td colspan="4">			
			<br>
			<%
				...
				{
			%>
				<input type="hidden" name="bankRegion<%=keyStr%><%=BANKN%>" value="<%=PROVZ%>">
...
%>
		...
		%>
			...
			<%
				...
				{
			%>
				<input type="hidden" name="bankStreet<%=keyStr%><%=BANKN%>" value="<%=STRASBank%>"><%=STRASBank%>
			<%
				}
			%>
			</Td>
		...
		<Tr>	
			...
			<%
				...
				{
			%>
				<input type="hidden" name="bankCity<%=keyStr%><%=BANKN%>" value="<%=ORT01Bank%>"><%=ORT01Bank%>
			<%
				}
			%>
			</Td>
		...
		<Tr>	
			...
			<%
				...
				{
			%>
				<input type="hidden" name="bankBranch<%=keyStr%><%=BANKN%>" value="<%=BRNCH%>"><%=BRNCH%>
			<%
				}
			%>
			</Td>
		...
		%>
			...
			<%
				...
				{
			%>
				<input type="hidden" name="bankIFSCCode<%=keyStr%><%=BANKN%>" value="<%=BKREF%>"><%=BKREF%>
			<%
				}
			%>
			</Td>
...
<Div class="col-xs-12 col-md-8 col-md-offset-4" >
	...
	<!--<button type="button" class="btn btn-primary" onclick="funUploads()">View/Add Uploads</button>-->
