...
<%@include file="../../../Includes/JSPs/Misc/iTempVendList.jsp"%>               
<%//@ include file="../Misc/ezCommonMethods.jsp"%> 
   ...
   <div class="box box-primary">
      ...
      <form name="myForm" method="post"> 
		...
		<Tr>
			...
			<Td>
				<label for="venType"> Type of Vendor </label>  
			</Td>			
			<Td>
				<div class="input-group">
		...
		<Th> Status of vendor </Th>
		<Td>	
				  <div class="input-group">
					 <select class="form-control input-sm" name="type" id="type">

						<option value="ALL" selected>All</option>
						<option value="Reg">To Register</option>
						<option value="Res">To ReSubmit</option>						
			...
			for(int i=0;i<Count;i++)   
                        {
                        		...
                        		docStatus = hdrXML.getFieldValueString(i,"ECFH_STATUS");
...
%>	 	
                     <tr>
                     	<td align="center">
<%
				if("IN".equals(venType) || "IM".equals(venType))
				{
					if(isNABuyer)
					{
%>
						<a style='text-decoration:underline' href="ezViewTempNAVend.jsp?docId=<%=hdrXML.getFieldValueString(i,"ECFH_DOC_ID")%>"><%=hdrXML.getFieldValueString(i,"ECFH_DOC_ID")%></a>
<%
					}
					else
					{
%>
						<a style='text-decoration:underline' href="ezViewTempVend.jsp?docId=<%=hdrXML.getFieldValueString(i,"ECFH_DOC_ID")%>"><%=hdrXML.getFieldValueString(i,"ECFH_DOC_ID")%></a>
<%
					}
				}
%>
			...
			<td align="center"><%=createdBy%></td>
                     	<td align="center"><%=sdf.format((Date)hdrXML.getFieldValue(i,"ECFH_CREATED_ON"))%></td>
                     	<td align="center"><%=sdf.format((Date)hdrXML.getFieldValue(i,"ECFH_MODIFIED_ON"))%></td>
			<td align="center"><%=docStatus%></td>	
                	                <Td  align="center">
						<a href = "#" onclick="ezReminder('<%=qcfNo%>','<%=hdrXML.getFieldValueString(i,"ECFH_CUST_NAME").toUpperCase()%>','<%=docStatus%>')"> <font color="red"><i class="fa fa-arrow-left" ></i></font></a> 
		</Td> 
...
<script type="text/javascript" class="init"> 
   $(document).ready( function () {
	...
	function deleteQcf(qcfNo)
	{
   	...
   	function ezSubmit()
   	{
		document.myForm.action = "ezTempVendList.jsp";
	...
	function ezReminder(vendor,vendName,status)
	{
		var msgTxt="Are you sure you want to Send Reminder to Vendor?";
		if(confirm(msgTxt))
		{	

			$.ajax({
				url: "../Misc/ezRemindVNR.jsp?docId="+vendor+"&vendName="+vendName+"&status="+status, 
				type: "POST",    
				dataType:"text",   
				error: function (response) 
				{ 
				...
				success: function (response) 
				{
				alert("Reminder for Vendor:"+vendor+" has been sent successfully");
				//document.myForm.submit();
				}
			});
		}
