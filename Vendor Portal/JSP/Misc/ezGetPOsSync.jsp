...
<%
	String site = "300"; //request.getParameter("site");
	ezc.ezcommon.EzLog4j.log(">>>>>>>>>>>>StartOfPOSYNC JOB>>>>>>>>>>>>>>>:","I");
	...
	out.println("INTO POS PAGE");
	ezc.ezcommon.EzLog4j.log(">>>>>>>>>>>>MIDOFPOSYNC HERE>>>>>>>>>>>>>>>:","I");
	try
	{
	...
	try
	{
		...
		ezc.ezcommon.EzLog4j.log(">>>>>>>>>>>>FROM_DATE22>>>>>>>>>>>>>>>:"+sapPurOrd.getValue("FROM_DATE"),"D");
		ezc.ezcommon.EzLog4j.log(">>>>>>>>>>>>TO_DATE22>>>>>>>>>>>>>>:"+sapPurOrd.getValue("TO_DATE"),"D");
		...
		compCodeTab.setValue("SIGN","I");
		compCodeTab.setValue("OPTION","EQ");
		compCodeTab.setValue("LOW","7300");

		compCodeTab.appendRow();
		compCodeTab.setValue("SIGN","I");
		compCodeTab.setValue("OPTION","EQ");
		compCodeTab.setValue("LOW","7501");

		compCodeTab.appendRow();
		compCodeTab.setValue("SIGN","I");
		compCodeTab.setValue("OPTION","EQ");
		compCodeTab.setValue("LOW","9000");*/
		...
		poDocTypeTab.setValue("SIGN","I");
		poDocTypeTab.setValue("OPTION","EQ");
		poDocTypeTab.setValue("BSART_LOW","ZH07");
		poDocTypeTab.appendRow();
		poDocTypeTab.setValue("SIGN","I");
		poDocTypeTab.setValue("OPTION","EQ");
		...
		JCoTable relInd   	= sapTabParam.getTable("REL_INDICATOR");
		relInd.appendRow();
		relInd.setValue("SIGN","I");
		relInd.setValue("OPTION","EQ");
		relInd.setValue("LOW","R");
		
		relInd.appendRow();
		relInd.setValue("SIGN","I");
		relInd.setValue("OPTION","EQ");
		relInd.setValue("LOW","J");
		
		relInd.appendRow();
		relInd.setValue("SIGN","I");
		relInd.setValue("OPTION","EQ");
		relInd.setValue("LOW","S");
		ezc.ezcommon.EzLog4j.log(">>>>>>>>>>>>BEfore Execution JOB>>>>>>>>>>>>>>>:","I");
		...
		try {
			...
			if (poCount>0)
			{
				do{
					...
					ezc.ezcommon.EzLog4j.log("PO_PLANTS FROM SAP>>>>>>>"+sapPoList.getValue("SUPPL_PLNT"),"I");
					try{
						poRelHdrXML.setFieldValue("PLANT",sapPoList.getValue("SUPPL_PLNT"));
						//ezc.ezcommon.EzLog4j.log("PO_PLANTS FROM SAP>>>>>>>"+sapPoList.getValue("SUPPL_PLNT"),"I");
					}catch(Exception e){
						ezc.ezcommon.EzLog4j.log("EXCEPTION OCCURED WHILE GETTING PLANT CODES SAP>>>>>>>"+e,"E");
					}	
			
					poRelHdrXML.setFieldValue("ORDERDATE", sapPoList.getValue("CREATED_ON"));
		...
		{
			ezc.ezcommon.EzLog4j.log("Exception in Calling ZEZ_UNRELEASED_POS>>>"+e,"I");
		}
	}
	catch(Exception e)
	{
	...
	if(poRelHdrXML!=null)
	    ...
	    ezc.ezcommon.EzLog4j.log("poRelHdrXMLCount>>>"+poRelHdrXMLCount,"I");
	...
	session.invalidate();
	out.print("GET_POS_FROM_SAP_END11>>>>>>>>"+poRelHdrXML.toEzcString());	
