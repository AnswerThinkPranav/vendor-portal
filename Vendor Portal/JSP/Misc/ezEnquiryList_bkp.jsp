...
<% 	
 	String docId ="",city="",name1="",email="",mob="",state="",landline="",loc="";
   	if(QueryCnt>0)
   	{
...
<Script>
	...
	%>
		<Tr>
			<Td width="8%" filter="false"><a href="javaScript:onClick=funUpload()"><%=docId%></a></Td>
			...
			<Td width="10%" filter="false"><button onclick="funCreVen()">Create</button></Td>
		</Tr>	
	<%
		}
	%>	
...
<script src="../../Library/jquery/jquery-validation-1.16.0/dist/additional-methods.min.js"></script>
 ...
 function funUpload()
 {
 	...
 	var loc='<%=loc%>';
 	var Url="../../../../../ezEnquiry.jsp?city="+city+"&name1="+name1+"&mob="+mob+"&email="+email+"&state="+state+"&landline="+landline+"&loc="+loc;
 		$.fancybox.open({
 		href : Url,
 		type : 'iframe',
 		padding : 5,
 ...
 function funCreVen()
  {
 	...
 	var mob='<%=mob%>';
 	var state='<%=state%>';
    	document.myForm.action="ezTempUserCreation.jsp?docId="+docId+"&loc="+loc+"&name1="+name1+"&email="+email+"&state="+state+"&mob="+mob;
